    ...
    const fetchData = async () => {
      ...
      fetch(
        process.env.GATSBY_DRUPAL_HOST +
          `/jsonapi/field_config/field_config?api-key=${process.env.GATSBY_DRUPAL_KEY}&filter[drupal_internal__id]=paragraph.signposts.field_signpost`,
        requestHeaders
      )
        ...
        .then(async (data) => {
          ...
          for (const [, value] of Object.entries(
            data.data[0].attributes.settings.handler_settings.target_bundles
          )) {
            ...
            try {
              const { default: loadedDefault } = await import(
                `${importFilename}`
              )
