...
export default class DrupalMediaStore {
  ...
  async persist(files) {
    ...
    for (const { file } of files) {
      ...
      await axios.post(process.env.GATSBY_DRUPAL_HOST + `/api/tinacms/media/create`, formData, {
        headers: {
    ...
    const items = data.props.media.data.map(
      (mediaItem) => {
        return {
          ...
          src: process.env.GATSBY_DRUPAL_HOST + mediaItem.field_media_image.uri.url,
          previewSrc: process.env.GATSBY_DRUPAL_HOST + mediaItem.thumbnail.uri.url,
        }
      }
    )
    return {
...
export async function mediaData() {
  ...
  const currentRoute =
    `jsonapi/media?include=field_media_image,thumbnail&jsonapi_include=1&sort=-created`
